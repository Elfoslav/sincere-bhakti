...
export async function POST(request: NextRequest) {
  ...
  if (!await checkRateLimit(RATE_LIMIT_PREFIX.verifyEmail, ip, RATE_LIMITS.verifyEmail.limit, RATE_LIMITS.verifyEmail.windowMs)) {
    return NextResponse.json({ error: ERROR_TOO_MANY_REQUESTS }, { status: HTTP_TOO_MANY_REQUESTS });
  }

  try {
    const body = await request.json();
    const parsed = parseBody(body, verifyEmailSchema, "POST /api/auth/verify-email");
    ...
    // Wrong type (e.g. a reset token submitted here): invalid, but don't delete
    ...
    return serverError("POST /api/auth/verify-email", error);
  }
}
