...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!await checkRateLimit("search-channels", ip, RATE_LIMITS.searchChannels.limit, RATE_LIMITS.searchChannels.windowMs)) {
...
export async function POST(request: NextRequest) {
  ...
  if (!await checkRateLimit("create-channel", session.user.id, RATE_LIMITS.createChannel.limit, RATE_LIMITS.createChannel.windowMs)) {
    return NextResponse.json({ error: ERROR_TOO_MANY_REQUESTS }, { status: HTTP_TOO_MANY_REQUESTS });
  }
