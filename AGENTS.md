...
- **OG routes render dynamically** (rate limiting reads `headers()`), so they must set `Cache-Control` explicitly or every crawler hit pays a DB lookup + full render. Use the shared `OG_IMAGE_CACHE_CONTROL` (1h + SWR) for success and `OG_IMAGE_FALLBACK_CACHE_CONTROL` (short) for the fallback branches, so a temporarily degraded card self-heals quickly. For `ImageResponse` pass `{ ...size, headers: { "Cache-Control": ... } }`; for raw `sharp` responses set it on the `Response` headers.
