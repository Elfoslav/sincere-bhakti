...
- **No repeating code — reuse first.** Before writing any logic, component, constant, schema fragment, or service query, search for an existing shared version (`Grep`/`Glob`) and reuse it. Duplication is a bug: when the same logic exists in two places, a fix applied to one silently misses the other. Examples of what MUST be shared, not copied: feed scope/visibility filters (`src/lib/services/feed-scope.ts`), service error classes (`src/lib/services/errors.ts`), API route preambles (`requireVerifiedUser`, `resolveWriteChannel`, `mutationErrorResponse`, `parseLanguageParam` from `src/lib/api-helpers.ts`), Zod field fragments (`emailField`, `nameField`, `entityIdField`, `safeUrlField` from `src/lib/validation.ts`), URL builders (`src/lib/content-url.ts`), avatars/skeletons/error states (`ChannelAvatar`, `FeedLoadingSkeleton`, `ErrorState`).
- **Utility functions** go in `src/lib/` (e.g. `video.ts`, `auth.ts`, `validation.ts`, `rate-limit.ts`).
- **Extract pure utilities from components.** Any function inside a `.tsx` file that has no React hooks (`useState`, `useEffect`, etc.) and no browser-DOM dependency must be moved to `src/lib/` so it can be unit-tested without jsdom. Examples: `genId()`, `formatBytes()`, `getSiteUrl()`, `stripExcerptLinks()`, `getRandomInt()`.
- **Do not duplicate constants that are already exported from `src/lib/`.** If a list, threshold, or config constant exists in a lib file, import it — never redeclare it locally. This includes Zod field chains: reuse the shared fragments instead of re-declaring the same `.trim().min().max()` shape per schema.
...
- **Reusable UI** goes in `src/components/` or `src/components/ui/` (shadcn). Before building a new card, avatar, dialog, skeleton, empty-state, or error/loading page, check for an existing shared one and extend it with props (`size`, `variant`, `fallback`) instead of copying its markup.
- **One implementation per concept.** Sibling features (posts vs blog, feed vs detail, list vs picker) MUST share the underlying helper/hook/service and differ only in thin adapters (endpoint, namespace, URL builder). Cloning a whole file and renaming identifiers is never acceptable — extract the generic core first (see `useInfiniteFeed`, `FeedParams<T>`, `setEntityCategories`).
...
- Channels are multilingual: a `Channel` has one `ChannelTranslation` (name + slug) per language. **Names and slugs are unique PER LANGUAGE, not globally** — the DB enforces `@@unique([language, normalizedName])` and `@@unique([language, slug])` on `ChannelTranslation` (and `@@unique([language, oldSlug])` on `ChannelSlugHistory`). A single channel MAY reuse the same name/slug across its own language translations (e.g. an `en` and a `cs` translation both named "Devotees", both with slug `devotees`).
- **Slugs are unique per-language, so a slug lookup MUST be language-aware.** Resolve with the composite `language_slug` key first, then fall back to `findFirst({ where: { slug } })` for cross-locale links (e.g. `/cs/channels/<en-slug>`). Use the shared `findManageableTranslationBySlug(slug, language?)` helper in `channel.ts` for management routes (rename / add / delete translation); `getChannelBySlug` / `getChannelSettingsBySlug` / `resolveSlugRedirect` follow the same prefer-language-then-fallback pattern. Never write a bare `findUnique({ where: { slug } })` — it no longer typechecks and would be ambiguous. **Any management route that identifies a channel/translation by its URL slug MUST receive that slug's language and pass it to `findManageableTranslationBySlug(slug, language)`** — otherwise the helper falls back to an unordered `findFirst({ slug })` and, when the same slug string exists on two channels in different languages, can resolve the wrong channel (404 if unmanageable, or mutate the wrong one if manageable). The slug's language travels as `?language=` on the rename route (`PATCH /api/channels/[slug]`, which has no body language) and as `?slugLanguage=` on the translations route (`POST`/`DELETE /api/channels/[slug]/translations`, whose `language` already denotes the NEW/target translation).
- **Name AND slug both gate uniqueness on creation — reject, never auto-suffix.** Two distinct names can slugify alike within a language (e.g. `"Devotees!"` and `"Devotees"` → `devotees`). Channel creation, add-translation, rename, AND registration all reject with `name_taken` (409) when the derived per-language slug is already taken, even if the name itself is free — use the shared `isPerLanguageSlugTaken(client, language, slug)` probe (checks active translations + retired history in that language) and throw `NameTakenError` on a hit. Do NOT append `-2`/`-3` to a user-chosen slug. **The one exception** is `createPersonalChannel` — the defensive backfill called at login/first-post ([auth.ts](src/lib/auth.ts), [posts/route.ts](src/app/api/posts/route.ts)) — which MUST stay resilient (auto-suffix + UUID fallback) so a slug technicality can never break sign-in; it is not a user-initiated naming action.
...
- **A channel name is owned by ONE channel across ALL languages.** Even though the DB constraint is per-language, the *ownership* rule is broader and enforced in application code: if a name (its `normalizedName`) is already used by some channel in ANY language — active or in slug history — then no OTHER channel may take that name in ANY language. Only the channel that already owns the name may reuse it, and only for its own other-language translations. Concretely: the name check (`isNormalizedNameTaken`, and the inline `channelTranslation.findFirst({ where: { normalizedName } })` / `channelSlugHistory.findFirst({ where: { oldNormalizedName } })` probes in `createChannel`, register, and the users rename) is GLOBAL across languages and only ever excludes the current channel (`channelId: { not: <id> }`) — never scope it to a single language. In contrast, slug and slug-history collision probes ARE scoped to the language (`{ language, slug }` / `language_slug`). The user-facing error is "A channel with this name already exists." — do not phrase it as per-language or per-channel.
- **The global name-ownership check is a read-before-write, so it MUST run inside a transaction holding an advisory lock on the name** — otherwise two creators picking the same name in *different* languages both pass the check and both insert (no P2002, since the DB unique is per-language), leaving the name owned by two channels. Use `claimChannelName(tx, normalizedName, excludeChannelId?)` (lock + `isNormalizedNameTaken` throw) from `channel.ts`, or `lockChannelName(tx, normalizedName)` when the check is inline. It uses `pg_advisory_xact_lock` (transaction-scoped → auto-released, safe under connection pooling). Every name-claiming path is already wired: `createChannel`, `renameChannelTranslation` (covers rename + add-translation update), the add-translation create branch, `register`, and the users rename. Any NEW name-claiming path MUST take the lock inside its transaction, or it reopens the race.
- **The brand name is reserved in ALL languages.** `SINCERE_BHAKTI_NAME` (default "Sincere Bhakti") may only be used by the `SINCERE_BHAKTI_EMAIL` owner — in any language, on any channel/user rename or translation. Every name-accepting mutation (create channel, rename channel, add/rename a translation, register, rename user) MUST call `isBrandNameBlocked(name, callerEmail)` from `src/lib/validation.ts` and return `ERROR_NAME_TAKEN` (409) when it returns true. `isBrandName` matches the brand words as substrings, so it is inherently language-agnostic — do not add per-language exceptions.
...
- Current coverage:
  ...
  - `normalizeName()` — trims, collapses internal whitespace runs to a single space, folds diacritics, and lowercases (so `"Krishna  Das"`, `"Krishna\tDas"` and `"krishna das"` all compare equal) ✅
